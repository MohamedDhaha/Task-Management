...
// Initialize database connection variables (cached for subsequent requests)
let client
let db
let users
let tasks
 ...
 * Creates necessary indexes and seeds demo data if collections are empty.
 */
async function init() {
  if (db) return
  try {
    ...
    console.log(`[Database] Found ${userCount} users during init.`);
    if (userCount === 0) {
      console.log('[Database] No users found. Initializing demo data...');
...
/**
 * Retrieves all registered users with the role 'user'.
 * @returns {Promise<Array>} Array of user documents
 */
export async function getUsers() {
  await init()
  console.debug('[Database] Fetching all users');
 ...
 * @returns {Promise<Array>} Array of all task documents
 */
export async function getTasks() {
  await init()
  console.debug('[Database] Fetching all tasks');
 ...
 * @returns {Promise<Array>} Array of user-specific task documents
 */
export async function getTasksByUserId(userId) {
  await init()
  console.debug(`[Database] Fetching tasks for userId: ${userId}`);
